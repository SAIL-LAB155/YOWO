...
def fill_truth_detection(labpath, w, h, flip, dx, dy, sx, sy):
    ...
    if os.path.getsize(labpath):
        ...
        for i in range(bs.shape[0]):
            cx = (bs[i][1] + bs[i][3]) / (2 * 2048)
            cy = (bs[i][2] + bs[i][4]) / (2 * 1536)
            imgw = (bs[i][3] - bs[i][1]) / 2048
            imgh = (bs[i][4] - bs[i][2]) / 1536
            bs[i][0] = bs[i][0] - 1
            bs[i][1] = cx
            bs[i][2] = cy
            bs[i][3] = imgw
...
def load_data_detection(base_path, imgpath, train, train_dur, shape, dataset_use='ucf101-24', jitter=0.2, hue=0.1, saturation=1.5, exposure=1.5):
    ...
    im_split = imgpath.split('/')# drown, drown_01, 00020.txt
    ...
    if dataset_use == 'ucf101-24' or dataset_use == 'swim_drown':
