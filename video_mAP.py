...
base_path     = data_options['base']#./data/ucf24
testlist      = os.path.join(base_path, 'ucf24_tmp.txt')
...
def get_clip(root, imgpath, train_dur, dataset):
    im_split = imgpath.split('/')#./data/ucf24/rgb-images/Basketball/v_Basketball_g01_c01/00009.jpg
    num_parts = len(im_split)
    class_name = im_split[-3]
    file_name = im_split[-2]
    im_ind = int(im_split[num_parts - 1][0:5])
    if dataset == 'ucf101-24':
        img_name = os.path.join(class_name, file_name, '{:05d}.jpg'.format(im_ind))#Basketball/v_Basketball_g01_c01/00009.jpg
    ...
    img_folder = os.path.join(base_path, 'rgb-images', class_name, file_name)#./data/ucf24/rgb-images/Basketball/v_Basketball_g01_c01
...
class testData(Dataset):
    def __init__(self, root, shape=None, transform=None, clip_duration=16):
        ...
        self.root = root#./data/ucf24/rgb-images/Basketball/v_Basketball_g01_c01/00009.txt
        if dataset == 'ucf101-24':
            self.label_paths = [root.replace('.txt', '.jpg')]#sorted(glob.glob(os.path.join(root, '.jpg')))
        elif dataset == 'jhmdb-21':
    ...
    def __getitem__(self, index):
        ...
        label_path = self.label_paths[index]#./data/ucf24/rgb-images/Basketball/v_Basketball_g01_c01/00009.jpg
...
def video_mAP_ucf():
    ...
    for line in lines:
        print(line)#Basketball/v_Basketball_g01_c01/00009.txt
        line = line.rstrip()
