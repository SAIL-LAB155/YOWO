...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    tx         = torch.zeros(nB, nA, nH, nW) #[4,5,7,7]
    ...
    for b in xrange(nB):
        ...
        for t in xrange(50):
            ...
            # print(gw, anchors[anchor_step*best_n])
            # print(gh, anchors[anchor_step*best_n+1])
            tw[b][best_n][gj][gi] = math.log((gw+sys.float_info.epsilon)/anchors[anchor_step*best_n])
            th[b][best_n][gj][gi] = math.log((gh+sys.float_info.epsilon)/anchors[anchor_step*best_n+1])
...
class RegionLoss(nn.Module):
    ...
    def __init__(self, num_classes=0, anchors=[], batch=16, num_anchors=5):
    ...
    def forward(self, output, target):
        ...
        t0 = time.time()
        nB = output.data.size(0)#4
        nA = self.num_anchors#5
        nC = self.num_classes#3
        nH = output.data.size(2)#7
        nW = output.data.size(3)#7

        # resize the output (all parameters for each anchor can be reached)
        output   = output.view(nB, nA, (5+nC), nH, nW)#[4, 5, 8, 7, 7]
        ...
        pred_boxes = convert2cpu(pred_boxes.transpose(0,1).contiguous().view(-1,4))#[980, 4]
        t2 = time.time()
