...
basepath      = data_options['base']#./data/swim_drown
trainlist     = data_options['train']
testlist      = data_options['valid']#./data/swim_validlist.txt
    ...
    for batch_idx, (frame_idx, data, target) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            for i in range(output.size(0)):
                ...
                else: #dataset_use == 'swim_drown
                    detection_path = os.path.join('swim_detections', 'detections_'+str(epoch), frame_idx[i])
                    current_dir = os.path.join('swim_detections', 'detections_'+str(epoch))
                    os.makedirs(current_dir, exist_ok=True)
                    # if not os.path.exists('swim_detections'):
                ...
                with open(detection_path, 'w+') as f_detect:
                    for box in boxes:
                        x1 = round(float(box[0]-box[2]/2.0) * 2048.0)
                        y1 = round(float(box[1]-box[3]/2.0) * 1536.0)
                        x2 = round(float(box[0]+box[2]/2.0) * 2048.0)
                        y2 = round(float(box[1]+box[3]/2.0) * 1536.0)
